...
app.post("/registar", async (req, res) => {
  ...
  try {
    ...
    db.query(sql, [nome, data_nasc, genero, email, nif, validade_cc, Password_hashed], (err) => {
...
app.post("/login", (req, res) => {
  ...
  const sql = "SELECT id_eleitor, nome_completo, palavra_passe FROM Eleitor WHERE NIF = ?";
  db.query(sql, [nif], async (err, results) => {
    ...
    if (results.length > 0) {
      ...
      if (match) {
        res.json({ success: true, nome: eleitor.nome_completo, idEleitor: eleitor.id_eleitor });
      } else {
...
app.post("/votar", (req, res) => {
  ...
  // 1. Definimos a query de Participação
  const sqlParticipacao = "INSERT INTO Participacao (id_eleicao, id_eleitor, data_voto) VALUES (?, ?, NOW())";
  
  // 2. DEFINIMOS A QUERY DE VOTO (O que faltava!)
  const sql = "INSERT INTO Voto (id_eleitor, id_candidato, id_eleicao) VALUES (?, ?, ?)";
// O Trigger vai funcionar PERFEITAMENTE. 
  ...
  db.query(sqlParticipacao, [id_eleicao || 1, idEleitor], (err) => {
    ...
    db.query(sqlVoto, [id_candidato, id_eleicao || 1], (votoErr) => {
      if (votoErr) {
        ...
        return res.status(500).json({ success: false, message: "Erro ao gravar o voto." });
      }
      res.json({ success: true });
    });
  });
